  ...
  const fetchUser = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_URL}/userRoutes/user-detail/${id}`, 
        {
  ...
  const follow = async ()=>{
    try {
        var response = await axios.post(
            process.env.REACT_APP_URL + "/userRoutes/follow/" + id,
          ...
          <div className="card" style={{ borderRadius: '15px' }}>
            <div className="card-body ">
              ...
              {!getCookie("id") === props.id &&
                ...
                &nbsp;
                <button type="button" className="btn  flex-grow-1 bg-4" onClick={follow}>{"UPDATE  "}</button>
              </div>
            </div>
          </div>
        </div>
